  ...
  .get(function (req, res)
  {
    ...
    https.get(fullURI, (resp) => 
    {
      ...
      resp.on('end', () => {
        ...
        //stock name is a real-world stock, check if it was liked to see if db stuff needs to be done
        else 
        { 
          ...
          db.collection('stocks').findOne({name:requestedStock}, (error, findOneResult)=>
          {
            ...
            else if (req.query.like)
            {
              ...
              ip = ip.split(',')[0]
              //null means collection doesn't exist or no stock doc found in db, in which case we add
              ...
              //The stock exists in the db. I need to see if the IP is listed in the "likes" array. If it is, do nothing, otherwise add the IP to the array and update the document.
              else 
              { 
                ...
                //A like doesn't exist yet for this ip, so update stock and return updated info
                else 
                { 
            ...
            //stock info was given without a like. Need to check if it exists in db. If it does, return "likes" info from db, otherwise just return 0 for likes.
            else 
            { 
              if(findOneResult==null)
              {
